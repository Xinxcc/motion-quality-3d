...
path_csv = r"F:\studium\Masterarbeit\projectcode\mediapipe_output_landmarks.csv"
...
for idx, video_name in enumerate(video_name_list):
    ...
    # 切片
    if idx == 0:
        splits = [0, 180,  -1]  # correct
    if idx == 1:
        splits = [0, 180,  -1]  # feet too wide
    split_data = []
    normalized_split_data = []
    for i in range(len(splits[:-1])):
        tmp = np.hstack((np.array([current_df.iloc[splits[i]:splits[i + 1]].iloc[:, joints * 4 + 1]]).squeeze(),
                         np.array([current_df.iloc[splits[i]:splits[i + 1]].iloc[:, joints * 4 + 2]]).squeeze(),
                         np.array([current_df.iloc[splits[i]:splits[i + 1]].iloc[:, joints * 4 + 3]]).squeeze()))
        split_data.append(tmp)
        normalized_df = current_df.apply(zscore)
        normalized_split_data.append(np.array([normalized_df.iloc[splits[i]:splits[i + 1]].iloc[:, joints * 4 + 1],
                                               normalized_df.iloc[splits[i]:splits[i + 1]].iloc[:, joints * 4 + 2],
                                               normalized_df.iloc[splits[i]:splits[i + 1]].iloc[:, joints * 4 + 3]]))
...
for t in range(1, 6):
    ...
    dtwvis.plot_warpingpaths(s1, s2, mpaths, best_mpath)
...
for e in range(3):
    for t in range(4):
        ...
        ax.bar(np.arange(0, 6) + width, normalized_idtw[6:12], width=width)
        ax.bar(np.arange(0, 6) + 2 * width, normalized_idtw[12:], width=width)
        ax.set_xticks(np.arange(0, 6) + 0.5 * width,
                      labels=['head', 'torso', 'left_arm', 'right_arm', 'left_leg', 'right_leg'], fontsize=14)
        ax.set_ylim(0, 20)
        plt.yticks(fontsize=14)
        ax.legend(('x', 'y', 'z'), loc='upper left', fontsize=18)
        f.savefig(r"F:\studium\Masterarbeit\videos" + "\\test_exercise_3d" + str(e) + "_sample" + str(t) + ".png")
